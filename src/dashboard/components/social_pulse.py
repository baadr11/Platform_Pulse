...
def _render_kpi_cards(raw_df: pd.DataFrame) -> None:
    ...
    if "text" in raw_df.columns:
        positive_kw = ["ممتاز", "رائع", "سريع", "شكرا", "جيد", "excellent", "great", "fast", "good"]
        pos_count = raw_df["text"].str.lower().apply(
            lambda t: any(w in str(t) for w in positive_kw)
        ).sum()
...
def _render_volume_chart(raw_df: pd.DataFrame) -> None:
    ...
    if raw_df["date"].isna().all():
        return
    raw_df = raw_df.dropna(subset=["date"])
    raw_df["month"] = raw_df["date"].dt.to_period("M").dt.to_timestamp()
    group_cols = [c for c in ["month", "sector"] if c in raw_df.columns]
    vol = raw_df.groupby(group_cols).size().reset_index(name="count")
    fig = px.bar(
        vol, x="month", y="count",
    ...
    fig.update_layout(title=dict(text="حجم الرسائل الشهري", font=dict(size=16, color="#f1f5f9")), height=360, **_DARK_NO_AXES)
    _apply_axes(fig)
    st.plotly_chart(fig, use_container_width=True)


def _render_no_data_placeholder() -> None:
    st.markdown("""
...
def _render_sample_table(raw_df: pd.DataFrame, n: int = 20) -> None:
    ...
    cols_needed = [c for c in ["date", "platform", "channel_or_tag", "sector", "text"] if c in raw_df.columns]
    sample = raw_df.sort_values("date", ascending=False).head(n)[cols_needed].rename(columns={
        "date": "التاريخ", "platform": "المنصة",
        "channel_or_tag": "القناة / الوسم",
        "sector": "القطاع", "text": "نص الرسالة",
    })
...
def render_social_pulse(filters: dict) -> None:
    ...
    if "date" in raw_df.columns:
        ...
        if date_from is not None or date_to is not None:
            ...
            if date_from is not None:
                filtered_date = filtered_date[filtered_date["date"] >= pd.Timestamp(date_from)]
            if date_to is not None:
                filtered_date = filtered_date[filtered_date["date"] <= pd.Timestamp(date_to)]
            if not filtered_date.empty:
                raw_df = filtered_date
    ...
    with col_left:
        agg_df = _load_social_aggregated()
        if agg_df is not None and not agg_df.empty:
            if sector_filter != "الكل":
                agg_df = agg_df[agg_df["sector"] == sector_filter]
            if not agg_df.empty:
                fig = px.line(
                    agg_df, x="date", y="social_score", color="sector",
                    markers=True,
            ...
            st.info("شغّل الجامع لعرض المؤشر الشهري المُجمَّع.")
    ...
    with col_right:
        ...
        if comp_df is not None and not comp_df.empty:
            ...
            st.info("شغّل compute_composite_index() لعرض مساهمة المكوّن الاجتماعي.")
