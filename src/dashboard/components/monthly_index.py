...
def _apply_filters (df :pd .DataFrame ,filters :dict )->pd .DataFrame :
    ...
    if filters .get ("sector")and filters ["sector"]!="الكل"and "sector"in df .columns :
        ...
        mask =(df ["sector"].astype (str ).str .contains (sec ,na =False )|
...
def render_monthly_index (filters =None ):
    ...
    if os .path .exists (composite_path ):
        ...
        "estimated_workers":(values *2800 ).astype (int ),
    ...
    kpi_style =(
    "border-radius:14px; padding:20px 16px; direction:rtl; "
    "border:1px solid rgba(255,255,255,0.07);"
    )
    with c1 :
        st .markdown (
        f'<div style="background:linear-gradient(135deg,#0f3460,#1a4a7a);{kpi_style }">'
        f'<p style="margin:0;font-size:0.78rem;color:#64748b;letter-spacing:.06em">مؤشر نبض المنصات</p>'
        f'<h2 style="margin:8px 0 0 0;font-size:2.2rem;color:#f1f5f9;font-weight:800">'
    ...
    with c2 :
        ...
        f'<p style="margin:0;font-size:0.78rem;color:#64748b;letter-spacing:.06em">التغير الشهري</p>'
        f'<h2 style="margin:8px 0 0 0;font-size:2.2rem;color:{delta_color };font-weight:800">'
        f'{change_pct_label }</h2></div>',unsafe_allow_html =True )
    with c3 :
        st .markdown (
        f'<div style="background:linear-gradient(135deg,#1e1b4b,#2d2a6e);{kpi_style }">'
        f'<p style="margin:0;font-size:0.78rem;color:#64748b;letter-spacing:.06em">العاملون المقدّرون</p>'
        f'<h2 style="margin:8px 0 0 0;font-size:1.9rem;color:#f1f5f9;font-weight:800">'
        f'{estimated_workers :,}</h2></div>',unsafe_allow_html =True )
    with c4 :
        st .markdown (
        f'<div style="background:linear-gradient(135deg,#1a1a2e,#2a2a4e);{kpi_style }">'
        f'<p style="margin:0;font-size:0.78rem;color:#64748b;letter-spacing:.06em">فترة الثقة 95%</p>'
        f'<h2 style="margin:8px 0 0 0;font-size:1.6rem;color:#94a3b8;font-weight:700;'
