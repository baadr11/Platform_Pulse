...
api_hash :str ="",
    ...
    try :
        ...
        session_string =session_string or _st .secrets .get ("TELEGRAM_SESSION_STRING","")
    except Exception :
        pass 
    if session_string :
...
def collect_twitter (since :Optional [datetime ]=None )->List [Dict [str ,Any ]]:
            ...
            for hashtag in TWITTER_HASHTAGS :
                ...
                try :
                    ...
                    for tweet in tweets :
                        ...
                        except (IndexError ,KeyError ,TypeError )as inner_exc :
                            logger .debug ("Twitter skipping malformed tweet for %s: %s",hashtag ,inner_exc )
                            continue 
                    logger .info ("Twitter hashtag %s (%s): %d tweets",hashtag ,sector ,len (tweets ))
                except Exception as exc :
    ...
    if _twitter_fallback_active or not all_records :
        if os .path .exists (SAMPLE_PATH ):
            try :
                ...
                logger .info ("Twitter loaded %d cached records from %s",len (sample_df ),SAMPLE_PATH )
    ...
    if all_records :
        logger .info ("Twitter total: %d tweets",len (all_records ))
    return all_records 

def _is_valid_telegram_id (raw_id )->bool :
    """Check if a Telegram API ID is a valid integer (not a placeholder)."""
    if raw_id is None :
        return False 
...
def run_social_collector (since :Optional [datetime ]=None )->pd .DataFrame :
    ...
    if not _is_valid_telegram_id (raw_api_id )or not raw_api_hash or raw_api_hash .strip ()in ("","YOUR_TELEGRAM_API_HASH_HERE"):
        logger .warning (
        "TELEGRAM_API_ID or TELEGRAM_API_HASH not set or is a placeholder — "
        ...
        try :
            ...
            logger .error ("TELEGRAM_API_ID must be an integer — skipping Telegram collection")
            telegram_api_id =0 
...
if __name__ =="__main__":
    ...
    if not result_df .empty :
        print (result_df .groupby (["platform","sector"]).size ().to_string ())
